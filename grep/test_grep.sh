  ...
  do
    ...
    do
      ...
      then
        ...
        echo "$TEST1"
        ./grep $TEST1 > my_text.txt
        grep $TEST1 > grep.txt
        DIFF_RES="$(diff -s my_text.txt grep.txt)"
        if [ "$DIFF_RES" == "Files my_text.txt and grep.txt are identical" ]
        then
          (( COUNTER_SUCCESS++ ))
        else
          echo "$TEST1" >> log.txt
        ...
        rm my_text.txt grep.txt

        TEST2="for grep.c $var $var2 $var3"
        echo "$TEST2"
        ./grep $TEST2 > my_text.txt
        grep $TEST2 > grep.txt
        DIFF_RES="$(diff -s my_text.txt grep.txt)"
        if [ "$DIFF_RES" == "Files my_text.txt and grep.txt are identical" ]
        then
